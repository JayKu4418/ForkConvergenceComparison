...
tc.clust2 <- tc[clust2,]
# I want to grab 70 samples from tc.clust1 randomly
rs <- sample(x= 1:nrow(tc.clust1.org),size = 70, replace=FALSE)
...
tc.1 <- cbind(tc.clust1,rep(x = "Similar",times=length(tc.clust1)))
colnames(tc.1) <- c(colnames(tc.1)[1:7],"Trough.Type")

tc.2 <- cbind(tc.clust2,rep(x = "Dissimilar",times=length(tc.clust2)))
...
wilcox.test(FirstDer.Corr ~ Trough.Type,alternative="greater",date=tc.fin,correct=FALSE)
...
# Number of tRNA genes found per base pair
...
wilcox.test(trna.ratio ~ trough.type, alternative = "less", data = all.trna, correct = FALSE)
